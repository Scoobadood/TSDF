 ...
 * @param physical_size
 */
GPUTSDFVolume::GPUTSDFVolume( const Eigen::Vector3i & size, const Eigen::Vector3f & physical_size ) : m_offset { Eigen::Vector3f::Zero()}, m_voxels {NULL}, m_weights {NULL} {
    std::cout << "TSDFVolume::ctor called. On entry m_voxels:= " << m_voxels << std::endl;
    ...
    std::cout << "TSDFVolume::ctor called. On exit m_voxels:= " << m_voxels << std::endl;
}
...
void TSDFVolume::set_size( uint16_t volume_x, uint16_t volume_y, uint16_t volume_z, float psize_x, float psize_y, float psize_z)
{
    ...
    std::cout << "TSDFVolume::set_size called. On entry m_voxels:= " << m_voxels << std::endl;
    ...
    std::cout << "TSDFVolume::set_size called. On exit m_voxels:= " << m_voxels << std::endl;
 ...
 */
Eigen::Vector3f TSDFVolume::offset( ) const
{
    return m_offset;
}
 ...
 * @return the data
 */
const float * TSDFVolume::data() const
{
    return m_voxels;
}
