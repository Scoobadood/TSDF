...
SceneFusion::SceneFusion( SceneFlowAlgorithm * sfa, RGBDDevice * rgbd_device ) {
	...
	RGBDDeviceCallback callback = std::bind( &SceneFusion::processFrames, this, _1, _2  );
 ...
 * Run SceneFusion
 */
void SceneFusion::processFrames( const DepthImage * depth_image, const PngWrapper * colour_image ) {
	std::cout << "processFrames Called" << std::endl;
	// Compute the scene flow
	Eigen::Vector3f translation;
	Eigen::Vector3f rotation;
	Eigen::Matrix<float, 3, Eigen::Dynamic> residuals;
	m_scene_flow_algorithm->computeSceneFlow( depth_image, colour_image, translation, rotation, residuals );
